...
Section2:CreateDropdown("Select Weapon", {
    List = Add(map(plr.Backpack:GetChildren(),function(a) return a.Name end),map(plr.Character:GetChildren(),function(a) return (a:IsA("Tool") and a.Name or nil) end)),
    Default = "Combat"
}, function(item)
...
end)local plr = game.Players.LocalPlayer
if not getgenv().AntiCheatBypassed then 
	local bypass
	for k,v in pairs(getgc(true)) do 
		if pcall(function() return rawget(v,"indexInstance") end) and type(rawget(v,"indexInstance")) == "table" and  (rawget(v,"indexInstance"))[1] == "kick" then
